...
class BranchCreator:
	...
	def checkout(self):
		...
		if self.on_branch not in remote_branches \
			...
			try:
				...
				if self.on_branch in remote_branches:
					print(remote_branches)
					return True
		elif self.on_branch in remote_branches and self.new_branch not in f"remotes/origin/{remote_branches}":
			return True
		...
		elif self.on_branch in f"remotes/origin/{remote_branches}" and self.new_branch in f"remotes/origin/{remote_branches}":
	...
	def create_branches(self):
		...
		if self.optional == "regular":
			...
			print(f"'{self.new_branch}' successfully created on '{self.on_branch}' in directory {self.repo_dir}")
		elif self.optional == "dynamic":
			self.git_root.branch(f"{self.new_branch}_dynamic",
			...
			print(f"'{self.new_branch}_dynamic' successfully created on '{self.on_branch}_dynamic' in directory {self.repo_dir}")
		else:
